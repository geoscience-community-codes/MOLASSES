...
/*DISTRIBUTE_PROPORTIONAL2SLOPE_LJC
		...
		The active Cell will SHARE ALL VOLUME of lava it contains ABOVE its RESIDUAL thickness.
*/
...
Algorithm:
	Do While there are more cells in ActiveList: active list gets built with each new pulse of lava
		Calculate excess lava in active cell
		IF active cell has lava to give:
			Identify 8 neighboring cells (N NE E SE S SW W NW)
	...
	For each active cell on UPDATE list:
		...
		remove all active list members
RETURN:
0 = SUCCESS
<0 = some ERROR
...
If bit-0 = 1 (0001 = 1) then a parent cell is SOUTH
If bit-1 = 1 (0010 = 2) then a parent cell is EAST
If bit-2 = 1 (0100 = 4) then parent cell is NORTH
If bit-3 = 1 (1000 = 8) then parent cell is WEST
0011 = 3 parent cell is SE
1001 = 9 parent cell is SW
0110 = 6 parent cell in NE
1100 = 12 parent cell is NW 
