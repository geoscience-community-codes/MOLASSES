  ...
  set_initial_seed ( seed1, seed2 );  /* Set seeds based on phrase. */
   ...
   /* Grid[row][col].active = 0; / vent cell */
    ...
    fprintf(stderr, "Final Distribute: %d cells inundated.\n\n", ActiveCounter);
		...
		fprintf(stdout, "ASCII flow file: %s\n", Out.ascii_flow_file);
		
    len = strlen(Out.raster_flow_file);
	  if (len > 2) {
	      ret = OUTPUT(
		    run,             /* run number */
		    raster_flow,      /* file output type */
		    &Out,            /* (type=Outputs*) 1D Output parameters structure */
		    &In,             /* (type=Inputs*) 1D Input parameters structure */
		    Grid,            /* (type = DataCell *) Global Data Grid */ 
		    &Vent,           /* (type=VentArr*) Vent Data structure */
	     ...
	     if (ret) fprintf(stdout, "Raster flow OUTPUT ERROR!\n");
	 }
	 fprintf(stdout, "Raster Flow file: %s\n", Out.raster_flow_file);
		...
		fprintf(stdout, "DEM re-initialized\n");
	} /* END:  for (run = start; run < (In.runs+start); run++) { */	
		
	len = strlen(Out.raster_post_dem_file);
	if (len > 2) {
		ret = OUTPUT(
		run,             /* run number */
	...
	fprintf(stdout, "Raster Post DEM file: %s\n", Out.raster_post_dem_file);
	
	len = strlen(Out.ascii_hits_file);
	if (len > 2) {
	  ret = OUTPUT(
		run,             /* run number */
	...
	fprintf(stdout, "Ascii Hits file: %s\n", Out.ascii_hits_file);
	
	len = strlen(Out.raster_hits_file);
	if (len > 2) {
	  ret = OUTPUT(
		run,             /* run number */
	...
	fprintf(stdout, "Raster Hits file: %s\n", Out.raster_hits_file);
