...
double *total_elev_diff) 
{
	...
	int Nrow, Srow, Wcol, Ecol, pRow, pCol;	/* neighbor Row and Col  relative to parent (active cell) */
	int neighborCount = 0;									/* Initialize neighbor counter */
	*total_elev_diff = 0.0; 								/* initialize total elevation difference */
	
	/* Calculate row and column locations for parent and neighbor cells */
	pRow = parent->row;
	pCol = parent->col;
	Nrow = pRow + 1;
	Srow = pRow - 1;
	...
	/*NORTH neighbor*/
	code = grid[pRow][pCol].parentcode & 4;
	if (!code) { /* NORTH cell is not the parent cell*/
#ifdef PRINT  
			fprintf(stderr,"\t NORTH-Cell * ");
#endif
			if (grid[pRow][pCol].eff_elev > grid[Nrow][pCol].prev_elev) { /* parent cell is higher than North neighbor */
				/* Calculate elevation difference between parent and neighbor */
				(neighborList+neighborCount)->elev_diff = grid[pRow][pCol].eff_elev - grid[Nrow][pCol].prev_elev;
				(neighborList+neighborCount)->row  = Nrow;
				(neighborList+neighborCount)->col  = pCol;
...
#ifdef PRINT  
			else fprintf(stderr, "NORTH neighbor too high [%0.4f]\n", grid[Nrow][pCol].prev_elev);
#endif
	}
#ifdef PRINT  
	else fprintf(stderr, "\tParent=NORTH[%d]\n", grid[pRow][pCol].parentcode);
#endif
	
	/*SOUTH*/
	code = grid[pRow][pCol].parentcode & 1;
	if (!code) { /* SOUTH cell is not the parent cell*/
#ifdef PRINT  
			fprintf(stderr,"\t SOUTH-Cell * ");
#endif			
			if(grid[pRow][pCol].eff_elev > grid[Srow][pCol].prev_elev) { /* parent cell is higher than SOUTH neighbor */
				/* Calculate elevation difference between parent and neighbor */
				(neighborList+neighborCount)->elev_diff = grid[pRow][pCol].eff_elev - grid[Srow][pCol].prev_elev;
	...
	/*EAST*/
	code = grid[pRow][pCol].parentcode & 2;
	if (!code) { /* EAST cell is not the parent cell*/
#ifdef PRINT  
			fprintf(stderr,"\t EAST-Cell * ");
#endif
			if (grid[pRow][pCol].eff_elev > grid[pRow][Ecol].prev_elev) { /* parent cell is higher than EAST neighbor */
				/* Calculate elevation difference between parent and neighbor */
				(neighborList+neighborCount)->elev_diff = grid[pRow][pCol].eff_elev - grid[pRow][Ecol].prev_elev;
				(neighborList+neighborCount)->row  = pRow;
	...
	/*WEST*/
	code = grid[pRow][pCol].parentcode & 8;
	if (!code) { /* WEST cell is not the parent cell*/
#ifdef PRINT  
			fprintf(stderr,"\tWEST-Cell * ");
#endif			
			if(grid[pRow][pCol].eff_elev > grid[pRow][Wcol].prev_elev) {/* parent cell is higher than WEST neighbor */
				/* Calculate elevation difference between parent and neighbor */
				(neighborList+neighborCount)->elev_diff = grid[pRow][pCol].eff_elev - grid[pRow][Wcol].prev_elev;
				(neighborList+neighborCount)->row  = pRow;
